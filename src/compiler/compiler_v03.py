...
def adjusted(initial_ion_pos, pos, idx):
    x, y = pos
    if pos == initial_ion_pos[0]:
        return (x, y + 1)
    if pos == initial_ion_pos[4]:
        return (x, y - 1)
    if pos in [initial_ion_pos[5], initial_ion_pos[6], initial_ion_pos[7]]:
        return (x - 1, y)
    elif pos in [initial_ion_pos[1], initial_ion_pos[2], initial_ion_pos[3]]:
        return (x + 1, y)
    raise ValueError(f"Qubit index {idx} out of supported range")


def select_int_point(initial_ion_pos, current_positions, interaction_positions, qu1, qu2):
    """
    ...
    try:
        qu1_pos = adjusted(initial_ion_pos, current_positions[qu1], qu1)
        qu2_pos = adjusted(initial_ion_pos, current_positions[qu2], qu2)
...
def get_qubits_paths(initial_ion_pos, current_positions, interaction_positions, qu1, qu2):
    """
    ...
    """
    int_idx, int_pos = select_int_point(initial_ion_pos, current_positions, interaction_positions, qu1, qu2)
    path1 = shortest_path(adjusted(initial_ion_pos, current_positions[qu1], qu1), interaction_positions[int_idx])
    path2 = shortest_path(adjusted(initial_ion_pos, current_positions[qu2], qu2), interaction_positions[int_idx])
    return [current_positions[qu1]] + path1, [current_positions[qu2]] + path2


def get_two_qubit_path(initial_ion_pos, current_ion_pos, interaction_points, qu1, qu2, counter):
    """
    ...
    """
    path1, path2 = get_qubits_paths(initial_ion_pos, current_ion_pos, interaction_points, qu1, qu2)
    ...
    stepwise_positions         = []
    stepwise_positions_flipped = []
    for i in range(max_len):
        step = list(current_ion_pos)  # Copy the current state
        step_flipped = list(current_ion_pos)
        step[qu1] = padded1[i]
        step[qu2] = padded2[i]
        if counter % 2 == 1: # odd, swap postions
            step_flipped[qu1] = padded2[i]
            step_flipped[qu2] = padded1[i]
        if counter % 2 == 0: # even, do not swap postions
            step_flipped[qu1] = padded1[i]
            step_flipped[qu2] = padded2[i]
        stepwise_positions.append(step)
        stepwise_positions_flipped.append(step_flipped)

    forward = [pos.copy() for pos in stepwise_positions]
    forward_flipped = [pos.copy() for pos in stepwise_positions_flipped]

    stepwise_positions = forward + list(reversed(forward_flipped))
...
def debug(use_Z, use_cont_MS):
    ...
    plt.savefig("two_qubit_circuit.pdf", dpi=500)
...
def main():
    ...
    initial_ion_pos = [(1, 0), (0, 1), (0, 3), (0, 5), (3, 6), (4, 5), (4, 3), (4, 1)]
    ...
    for circuit_column in batched_circuit:
        if is_single_qubit_op(circuit_column):
            ...
            two_qubit_path = get_two_qubit_path(
                initial_ion_pos, current_ion_pos, interaction_points,
                circuit_column[0][2][0], circuit_column[0][2][1], cnt_two_op)
